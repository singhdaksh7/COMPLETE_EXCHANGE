...
| Legal consent | Policy acceptance captured at signup (Terms/Privacy/Risk), versioned + ip/ua evidence; gate on financial actions; consent banner for existing users | ✅ | `legal.consent.ts`, `UserLegalAcceptance`; `legal-consent.test.ts`, `auth-validators.test.ts` | test (Stage 9A) | INR-only notice is informational (not a separate acceptance) | — |
| Support tickets | User↔admin ticket system: own-ticket scoping, RBAC (support.view/manage) admin actions, HTML-rejecting sanitization, rate-limited, audit-logged; internal notes hidden from users | ✅ | `support.user.service.ts`, `support.routes.ts`; `support-user-service.test.ts`, `support-validators.test.ts` | test (Stage 9A) | dedicated in-app notifications on reply are a follow-up | P3 |
| Wallet balance | INR balance sourced from the ledger (`AccountBalance`); wallet overview now includes networkless fiat; loading/empty/error states | ✅ | `wallet.service.ts`; `wallet-service.test.ts` | test (Stage 9A) | — | — |
| Feature hiding | Referral + API Management gated off in INR-only mode (frontend flags; no backend module) | ✅ | `lib/config.ts`, `nav.tsx` | build (Stage 9A) | re-enable behind flags on approval | — |
