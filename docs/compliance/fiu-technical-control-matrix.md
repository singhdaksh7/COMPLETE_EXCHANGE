...
| Single active user session | New login revokes prior sessions; old token rejected (`SESSION_REVOKED_BY_NEW_LOGIN`); `USER_PREVIOUS_SESSION_REVOKED` audited | Implemented | `auth.service.ts` (`enforceSingleActiveSession`); `middleware/authenticate.ts`; `AuditLog` (`auth.previous_session_revoked`) | BE | Idle-session timeout still a gap |
| Login location signal | Consented browser geolocation (reduced precision) stored on session + `auth.login` metadata; optional enforcement | Partial (consent-based, spoofable) | `auth.service.ts` (`sanitizeLocation`); `REQUIRE_LOGIN_LOCATION` → `LOCATION_REQUIRED`; `AuthSession.deviceInfo.location` | BE | Not fraud-proof; no reverse-geocoding; no fake city/state |
| User in-app alerts | Real events only (KYC / INR deposit / INR withdrawal incl. paid / security); no mock/fake rows | Implemented | table `Notification` (`INR_WITHDRAWAL_PAID` added); `/notifications`; Security Center audit feed | BE/CO | Email delivery best-effort (SES gated) |
